...
export class OAuthClient {
  ...
  async saveTokens(tokenResponse) {
    ...
    // ディレクトリがなければ作成（700: owner rwx のみ）
  ...
  /**
   * 認証コードフローを開始（PKCE S256）
   */
  async authorize() {
    const state = crypto.randomBytes(16).toString('hex');

    // PKCE: code_verifier を生成し、SHA-256 ハッシュから code_challenge を導出
    const codeVerifier = crypto.randomBytes(32).toString('base64url');
    const codeChallenge = crypto.createHash('sha256').update(codeVerifier).digest('base64url');
    // トークン交換時に使えるようインスタンスに保持
  ...
  /**
   * 認証コードをアクセストークンに交換（PKCE: code_verifier を含める）
   */
  async exchangeCodeForToken(code) {
    ...
    // PKCE が使われていれば code_verifier を付与してクリア
